...
const I18N = {
  zh: {
    ...
    netProfitHelp: "期末资产减去累计投入，以初始本金为 1。它直接展示回测期间实际赚到或亏掉的金额。",
    netProfitRateHelp: "(期末资产 - 累计投入) / 累计投入。它直接回答全部已投入本金最终赚了多少；先投 1 元、期末再投入 999999 元时，这个值会接近 0，而不是沿用早期 1 元的年化收益。",
    drawdownShort: "回撤",
    sharpeShort: "夏普",
  ...
  en: {
    ...
    netProfitHelp: "Ending assets minus invested capital, expressed as a multiple of initial capital. It is the actual amount earned or lost during the backtest.",
    netProfitRateHelp: "(Ending assets - invested capital) / invested capital. It shows profit on all contributed principal; investing 1 initially and 999,999 at the end makes this value nearly zero rather than inheriting the first unit's annual return.",
    drawdownShort: "MDD",
    sharpeShort: "Sharpe",
...
function investorMetricsForDisplay(metrics = {}) {
  const hasCashflow = metrics.cashflowMode && metrics.cashflowMode !== "none" && Number(metrics.cashflowCount || 0) > 0;
  const optionalNumber = (value) => {
    if (value === null || value === undefined || value === "") return NaN;
    const numeric = Number(value);
    return Number.isFinite(numeric) ? numeric : NaN;
  };
  const suppliedInvestedCapital = optionalNumber(metrics.investedCapital);
  const investedCapital = Number.isFinite(suppliedInvestedCapital)
    ? suppliedInvestedCapital
    : 1 + Number(metrics.cashflowTotal || 0);
  const suppliedTerminalValue = optionalNumber(metrics.terminalValue);
  const terminalValue = Number.isFinite(suppliedTerminalValue)
    ? suppliedTerminalValue
    : hasCashflow
      ? NaN
      : 1 + Number(metrics.totalReturn || 0);
  const suppliedNetProfit = optionalNumber(metrics.netProfit);
  const netProfit = Number.isFinite(suppliedNetProfit)
    ? suppliedNetProfit
    : Number.isFinite(terminalValue)
      ? terminalValue - investedCapital
      : NaN;
  const suppliedNetProfitRate = optionalNumber(metrics.netProfitRate);
  const netProfitRate = Number.isFinite(suppliedNetProfitRate)
    ? suppliedNetProfitRate
    : Number.isFinite(netProfit) && investedCapital
      ? netProfit / investedCapital
      : NaN;
  const suppliedMoneyWeightedReturn = optionalNumber(metrics.moneyWeightedReturn);
  const moneyWeightedReturn = Number.isFinite(suppliedMoneyWeightedReturn)
    ? suppliedMoneyWeightedReturn
    : hasCashflow
      ? NaN
...
function fmtOptionalMultiple(value) {
  return Number.isFinite(value) ? fmtMultiple(value) : "—";
}

function metricSignClass(value) {
...
function comparisonSortValue(entry, sortKey) {
  ...
  const investor = investorMetricsForDisplay(entry.result?.metrics || metrics);
  ...
  if (sortKey === "netProfitRate") return Number(investor.netProfitRate ?? -Infinity);
  return 0;
}
...
function renderMetrics() {
  const m = state.result.metrics;
  const investor = investorMetricsForDisplay(m);
...
function renderComparisonPanel(message = "") {
  ...
  const comparesCashflow = entries.some((entry) => normalizeCashflow(entry.cashflow).mode !== "none");
  const showInvestorColumns = comparesCashflow && comparisonViewMode() !== "overlap";
